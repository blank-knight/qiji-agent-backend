...
| 功能模块 | 具体能力 |
|----------|---------|
| 仪表盘 | 用户数、Token 消耗金额、7 日趋势图、今日数据 |
| 用户管理 | 增删改查、充值/扣除积分、开关自定义 Key 权限 |
| 代理管理 | 增删改查、设置配额、配置代理 API Key、三级代理树 |
| 邀请码管理 | 生成/编辑/禁用，控制次数和有效期 |
| 版本管理 | 发布新版本、强制更新开关 |
| Token 消耗记录 | 全量流水查询，按用户/模型筛选 |
| 权限管理 | 管理员账号、角色组、菜单规则（RBAC） |
...
| 功能模块 | 具体能力 |
|----------|---------|
| 仪表盘 | 我的用户数、剩余配额、邀请码统计 |
| 邀请码管理 | 生成自己的邀请码、查看使用情况 |
| 用户解绑 | 解除用户和自己的绑定关系 |
...
| 后台 | 地址 | 默认账号 |
|------|------|---------|
| 总后台 | `http://域名/admin.php` | `admin` / `123456` |
| 代理后台 | `http://域名/agent.php` | 需在总后台创建 |
