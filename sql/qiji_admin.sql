...
CREATE TABLE IF NOT EXISTS `fa_user` (
  ...
  `maxsuccessions` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '最大连续登录天数',
  `prevtime` int(10) DEFAULT NULL COMMENT '上次登录时间',
  `logintime` int(10) DEFAULT NULL COMMENT '登录时间',
  `loginip` varchar(50) DEFAULT NULL COMMENT '登录IP',
  `loginfailure` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '失败次数',
  `loginfailuretime` int(10) DEFAULT NULL COMMENT '登录失败时间',
...
CREATE TABLE IF NOT EXISTS `fa_agent` (
  ...
  `models` varchar(2000) NOT NULL DEFAULT '' COMMENT '自定义可用模型列表(逗号分隔,空=不限制)',
...
-- -------------------------------------------------------------------
-- 补充: 管理员日志表 / 附件表（原脚本遗漏，回流自 71f50ff 本地修复）
-- -------------------------------------------------------------------
